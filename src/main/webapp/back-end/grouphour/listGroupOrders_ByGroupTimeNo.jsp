...
<jsp:useBean id="listGroupOrders_ByGroupTimeNo" scope="request" type="java.util.Set<GroupOrderVO>" /> <!-- 於EL此行可省略 -->
...
<html>
<head>
<title>團課時間的團課訂單 - listGroupOrders_ByGroupTimeNo.jsp</title>
    ...
    <div id="content-wrapper" class="d-flex flex-column">
      ...
      <div id="content">
        ...
        <div class="container-fluid">
        	<!-- Page Heading -->
          	<h1 class="h3 mb-4 text-gray-800">團課時間-訂單</h1>
          		<!-- DataTales Example -->
...
<!-- 									<th>刪除</th> -->
								...
								<c:forEach var="groupOrderVO" items="${listGroupOrders_ByGroupTimeNo}" >
									<tr ${(groupOrderVO.groupOrderNo==param.groupOrderNo) ? 'bgcolor=#E8E8E8':''}><!--將修改的那一筆加入對比色-->
							...
							<%--                     <c:if test="${groupHourVO.groupTimeNo==groupOrderVO.groupTimeNo}"> --%>
							<%-- 	                </c:if>     --%>
							<%-- 	                <c:forEach var="groupClassVO" items="${groupClassSvc.all}"> --%>
							<%--                     <c:if test="${groupClassVO.groupClassNo==groupHourVO.groupClassNo}"> --%>
							<%-- 	                </c:if>${groupClassVO.groupClassNo} --%>
							<%-- 	                </c:forEach> --%>
							<%--                 </c:forEach> --%>
							<!-- 			</td> -->
							<%-- 			<td><c:forEach var="groupHourVO" items="${groupHourSvc.all}"> --%>
							...
							<!-- 			</td> -->
										...
										<td>
										  <FORM METHOD="post" ACTION="<%=request.getContextPath()%>/back-end/grouporder/groupOrder.do" style="margin-bottom: 0px;">
										    <input type="submit" value="修改" ${(groupOrderVO.groupOrderStatus==1)? 'disabled':''}> 
										    <input type="hidden" name="groupOrderNo"      value="${groupOrderVO.groupOrderNo}">
										    <input type="hidden" name="requestURL" value="<%=request.getServletPath()%>"><!--送出本網頁的路徑給Controller--><!-- 目前尚未用到  -->
										    <input type="hidden" name="action"	   value="getOne_For_Update"></FORM>
										</td>
<!-- 										<td> -->
<%-- 										  <FORM METHOD="post" ACTION="<%=request.getContextPath()%>/back-end/grouporder/groupOrderNo.do" style="margin-bottom: 0px;"> --%>
